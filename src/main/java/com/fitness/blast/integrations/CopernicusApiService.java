...
public class CopernicusApiService {
    ...
    public String getPointType(double lat, double lon) {
        ...
        try {
            ...
            ResponseEntity<CopernicusApiResponseDto> response = this.restTemplate.exchange(new URI(targetURL), HttpMethod.GET, entity, CopernicusApiResponseDto.class);

            if (response.getStatusCode() == HttpStatus.OK) {
                log.info(response.getBody().toString());
                response.getBody().getFeatures().get(0).getFeatureInfo().get(0).getValue();
            }
