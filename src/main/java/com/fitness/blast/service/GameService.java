...
public class GameService {
    ...
    private void isGameOver(Game game) {
        if (game.getUser1Points().size() < 2 || game.getUser2Points().size() < 2) {log.info("Not enought points. User1 has: " + game.getUser1Points().size() + "and User2 has: " + game.getUser2Points().size());
            return;
        }
    ...
    private void isPointCollected(Double lat, Double lon, Set<Point> points) {
        points.stream().forEach(point -> {
                    ...
                    if (isPointCollected) {point.setCollected(isPointCollected);}
